...
def main():
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for val_images, val_labels_batch in val_bar:  # 正确解包：(images, labels)
                outputs = net(val_images.to(device))

                # 计算单batch损失（累加总损失）
                loss = loss_function(outputs, val_labels_batch.to(device))  # 用当前batch的真实标签
        ...
        # 计算验证指标（基于所有batch的标签）
        val_loss = val_total_loss / val_num  # 总损失 / 总样本数
